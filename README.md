...
- `/services`: capabilities and service categories for Website, Mobile Apps, SaaS Products, CRM, Softwares, Branding, Dashboard, and ERP work. The navbar Services item exposes submenu links for `/services/saas-products`, `/services/templates`, `/services/ai-automations`, and a final “See all services” link back to `/services`; the compact right-side menu keeps those child links collapsed until Services is clicked.
...
- `/services/templates` and `/services/ai-automations`: coming-soon service pages with project inquiry CTAs while those tracks are prepared.
