...
- `/about`, `/testimonials`: company and social-proof content. The About page is a CMS-backed agency story, sequenced as a hero call-to-action, studio narrative, team introduction, trusted-partner proof, working principles, and a shared closing project CTA. Its team section uses the shared `Team` component with CMS-backed member data and presents each person as an open rounded-corner portrait profile with the member role, index, name, and a yellow rule in a service-catalogue-matched, translucent dark-glass caption panel over the lower portrait. Team portraits come from the CMS image path; Vishroy, Divesh, Pravesh, and Alice use the supplied local assets in `public/assets/img/team/`. Team profiles render as an auto-advancing, manually swipeable snap carousel with progress dots on phones. Mobile rotation is active only while the carousel is in view and scrolls its own horizontal container directly, so it cannot pull a visitor back to the team section after they continue down the page; on desktop, three portraits remain in view while scrolling through the final four-person track in one right-to-left step, ending at Pravesh without a trailing empty track position.
...
Immediately after the homepage hero, `src/components/home-impact/home-impact.tsx` reuses the `heroLabel`, `heroDescription`, and first three `heroStats` from `content/pages/home.json` as a high-contrast proof section. It uses the local `public/assets/img/about-studio.jpg` image beneath a dark, desaturated overlay, preserving readable proof content while keeping visual continuity with the other image-backed sections. Its three-column desktop / stacked mobile impact display uses the shared `HeroStats` `impact` layout, so the same values animate into view without creating a second source of truth. It reads the static homepage mapper instead of mounting a second Tina visual-editing subscription alongside the hero. The previous homepage partner marquee is not mounted there.

The homepage uses a client-focused sequence: hero, impact proof, service catalogue, recent-work strip, Open Slot 01, team, delivery process, Open Slot 02, case studies, testimonials, FAQ, and contact footer. The `Team` component is therefore visible on the homepage as a human-trust proof point while the dedicated `/about` route remains the fuller agency-story destination. Keep `ProjectsCta` indexes unique whenever a slot is added or moved.
