...
- `/services`: capabilities for Social Media Marketing, SaaS Products, Websites, Website Templates, Softwares, Mobile Apps, Branding, and AI Integrations & Automations. The desktop Services submenu is a wide, topbar-aligned eight-service grid with relevant Lucide icons; it links directly to every service page and includes a final overview link. When the topbar is compact, its hamburger sits left, the logo stays centered, and the Schedule a Free Call CTA remains right; the navigation drawer opens from the left and presents the same service links and icons. The custom language selector supports English and French only, with a white active option.
...
The homepage and `/services` page both use `src/components/service-catalogue/service-catalogue.tsx` as an in-place service explorer rather than a static stack of categories or a pricing table. `content/pages/services.json` is the source of truth for each category’s description, scoped capabilities, local service image and alt text, and four-step delivery plan; Tina exposes the same fields under Services Page. Both instances use `/assets/img/services-section.jpg` as an understated grayscale section background, layered under a dark gradient overlay so the catalogue remains readable. At every breakpoint, the white high-contrast panel uses the standard `max-w-[1400px]` content alignment shared with the topbar. The panel remains mounted when a service is selected, so only its list/detail content changes; the detail view includes the selected service information, plan, scope, CTA, and an All services back control. On desktop, the relevant visual occupies the right-hand column; it is hidden on mobile, keeping the compact detail view information-first. The homepage wrapper (`src/components/services/services.tsx`) only connects the shared catalogue to the `#services` anchor, so both locations cannot drift apart. The generic homepage `Process` timeline is intentionally not repeated below the `/services` explorer.
...
Pricing must be read through `getPricingPage()` rather than copied into components. The canonical service set is Social Media Marketing, SaaS Products, Websites, Website Templates, Softwares, Mobile Apps, Branding, and AI Integrations & Automations; keep the services catalogue, pricing category slugs, and project-brief options aligned to that order.
