...
The service-catalogue image source is `content/pages/services.json`: Social Media Marketing uses the Nibble social asset, Branding uses the Alara branding asset, Websites uses the ImaginePlus Ltd project image, and Website Templates uses the Moris Metrics project image. ImaginePlus Ltd is the current public name and `/project/imagine-plus-ltd/` is its case-study route; its existing live-site URL remains unchanged.

Service-catalogue project imagery is always rendered in colour. The showcase wraps its on-image service title, description, and focus list in the same rounded, near-black, thin-bordered caption panel used by team cards. Both catalogue variants normalize Dropbox share links with `getImageUrl()` before rendering, so CMS image fields can safely use the same share URLs as project content.
