...
function PlanCard({ plan, index, tinaField, rawPlan, isExpanded, onToggleDetails }: {
  plan: PricingPlan;
  index: number;
  ...
  return (
    <motion.div
      ...
      className={`plan-card relative flex min-w-full snap-center flex-col rounded-2xl border p-6 transition-all duration-300 sm:min-w-[400px] sm:p-8 md:min-w-[440px] lg:min-w-0 ${plan.featured
    ...
    >
      ...
      <div className="mt-auto space-y-3 lg:order-last">
        ...
        <Button
          ...
          icon={<ChevronDown className={`h-4 w-4 transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`} />}
          iconPosition="right"
          ...
          ariaLabel={`${isExpanded ? 'Hide' : 'Show'} ${plan.name} package details`}
...
function PlanDetails({ plan, tinaField, rawPlan }: {
  plan: PricingPlan;
  tinaField: (obj: any, field: string) => string | undefined;
  rawPlan: any;
}) {
...
export const Pricing = () => {
  ...
  const pricingCategories = content.pricingCategories.length > 0
  ...
  const handleCarouselScroll = () => {
    ...
    const cards = Array.from(carousel.querySelectorAll<HTMLElement>('[data-pricing-card]'));
    ...
    const closestIndex = cards.reduce((closest, card, index) => {
      ...
      return currentDistance < Math.abs(closestCenter - carouselCenter) ? index : closest;
    }, 0);
  ...
  return (
    <section className="pricing overflow-hidden bg-vish-bg px-6 py-24 md:px-12 md:py-32" id="pricing">
      <div className="max-w-[1400px] mx-auto">
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            aria-labelledby={`${tabItems[activeCategoryIndex]?.id || 'pricing'}-tab`}
            aria-label={`${activeCategory.label} pricing packages`}
          >
            {activeCategory.plans.map((plan, index) => {
              return (
                <PlanCard
                  key={`${activeCategory.slug || activeCategory.label}-${plan.name}`}
                  plan={plan}
                  index={index}
                  tinaField={tinaField}
                  rawPlan={activeRawPlans?.[index]}
                  isExpanded={isDetailsExpanded}
                  onToggleDetails={() => setIsDetailsExpanded((current) => !current)}
                />
              );
            })}
          </motion.div>
