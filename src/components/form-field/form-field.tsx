...
export const FormField = ({
  ...
  return (
    <div>
      <label htmlFor={id} className={`mb-3 block font-sans text-base font-medium text-white ${labelClassName}`}>
        {label}{required ? <span className={`ml-1 ${requiredIndicatorClassName}`}>*</span> : null}
      </label>
      ...
      {multiline ? (
        ...
        <input id={id} value={value} onChange={handleChange} type={type} placeholder={placeholder} required={required} autoComplete={autoComplete} className={`${fieldClassName} ${inputClassName}`} />
      )}
    </div>
  );
}
